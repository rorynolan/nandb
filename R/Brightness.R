...
#'   \code{\link{SmoothFilterB}}? If \code{filt = "median"},
#'   \code{\link{MedianFilterB}} is invoked with the option \code{na_count =
#'   TRUE}.
...
BrightnessTimeSeries <- function(mat3d, frames.per.set, tau = NA,
  ...
  brightnesses <- lapply(sets, Brightness, tau = tau, mst = mst) %>%
