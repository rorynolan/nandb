...
#'   calculating `cc_number` (via [autothresholdr::mean_stack_thresh()])? If so,
...
#'   cross-correlated numbers without filtering (`filt = NULL`) using this
...
#' @export
cc_number <- function(img, ch1 = 1, ch2 = 2, tau = NULL, thresh = NULL,
  ...
  cc_number_img(cc_n, thresh = thresh_atts, tau = tau, filt = filt)
}
...
cc_number_timeseries <- function(img, frames_per_set, ch1 = 1, ch2 = 2,
  ...
  tau %<>% unlist()
  attr(tau, "auto") <- tau_auto
  cc_number_ts_img(cc_n_ts, frames_per_set = frames_per_set,
                   thresh = thresh_atts, tau = tau, filt = filt)
}

cc_number_file <- function(path, ch1 = 1, ch2 = 2, tau = NULL,
  ...
  cc_n <- cc_number(path, ch1 = ch1, ch2 = ch2, tau = tau,
...
cc_number_timeseries_file <- function(path, frames_per_set,
  ...
  cc_n_ts <- cc_number_timeseries(path, ch1 = ch1, ch2 = ch2,
...
cc_number_folder <- function(folder_path = ".", ch1 = 1, ch2 = 2,
  ...
  purrr::map(file_names, cc_number_file, ch1 = ch1, ch2 = ch2,
...
cc_number_timeseries_folder <- function(folder_path = ".", frames_per_set,
  ...
  setwd(folder_path)
  file_names <- dir(pattern = "\\.tiff*$")
  purrr::map(file_names, cc_number_timeseries_file, ch1 = ch1, ch2 = ch2,
