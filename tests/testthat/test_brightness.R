...
test_that("brightness works", {
  ...
  expect_error(brightness(matrix(1:4, nrow = 2), "B"),
               "but has dimension 2")
})
...
context("brightness_time_series()")
test_that("brightness_time_series works", {
  ...
  two_channel_img <- abind::abind(img, img, along = 3)
  bts_2ch <- brightness_time_series(two_channel_img, "b", 10)
  ...
  expect_error(brightness_time_series(matrix(1:4, nrow = 2), "b"),
  ...
  ijtiff::write_tif(img, '50.tif')
  ijtiff::write_tif(img, '50again.tif')
  context("brightness_time_series_folder()")
  filesstrings::create_dir("tempwithintemp")
  ijtiff::write_tif(img, 'tempwithintemp/50.tif')
  brightness_time_series_file("tempwithintemp/50.tif", def = "B",
                              frames_per_set = 10)
  expect_true(stringr::str_detect(
                dir("tempwithintemp/brightness_time_series"),
  ...
  expect_true(all(
    paste0("50", c("_brightness_B_timeseries_",
      ...
      list.files("brightness_time_series")))
