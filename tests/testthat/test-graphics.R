...
test_that("matrix_raster_plot is ggplot", {
  ...
  expect_error(matrix_raster_plot(img),
               paste0("`mat` must be a matrix.+",
...
test_that("`which_interval()` errors correctly", {
  ...
  interval_mat <- tibble::tribble(~left, ~right,
                                  0, 0.1,
                                  0.1, 0.5,
                                  0.5, 0.75,
                                  0.75, 1) %>%
  ...
  expect_error(which_interval(numbers, interval_mat),
               paste0("`interval_mat` must be a two-column matrix where.+",
