test_that("Number works", {
  ...
  number <- Number(img, tau = 'auto', mst = 'Huang', filt = 'median',
                           verbose = TRUE)
  expect_equal(round(mean(number, na.rm = TRUE), 4), 19.3662)
...
test_that("NumberTimeSeries works", {
  ...
  expect_equal(round(mean(nts, na.rm = TRUE), 3), 18.3)
  nts <- NumberTimeSeries(img, 20, tau = NA, mst = 'Huang',
                          mcc = 2, verbose = TRUE, seed = 4)
  expect_equal(round(mean(nts, na.rm = TRUE), 3), 17.785)
  ...
  expect_equal(round(mean(nts.2ch, na.rm = TRUE), 3), 17.785)
...
test_that("Numbers works", {
  ...
  numbers <- Numbers(img.paths, mst = 'Huang', tau = 7, mcc = 2,
                     seed = 7)
  if (!stringr::str_detect(tolower(Sys.info()['sysname']), "windows")) {
    expect_equal(round(mean(unlist(numbers), na.rm = TRUE), 3), 21.921)
  }
  imgs <- lapply(img.paths, ReadImageData)
  numbers <- Numbers(imgs, mst = 'Huang', tau = NA, mcc = 2, seed = 7)
  if (!stringr::str_detect(tolower(Sys.info()['sysname']), "windows")) {
    expect_equal(round(mean(unlist(numbers), na.rm = TRUE), 3), 19.441)
  }
