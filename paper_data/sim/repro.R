lapply(c("nandb", "filesstrings", "stringr", "tidyverse", "xtable", "clipr",
         "magrittr", "EBImage", "rcompanion"), library, character.only = TRUE)
...
olig <- master %>%
  ...
  summarize(oligomerization = FiveTenMers(sim, brightness)) %>%
  ungroup %>%
