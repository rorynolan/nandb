required <- c("nandb", "filesstrings", "stringr", "tidyverse",
              "EBImage", "autothresholdr", "rcompanion")
...
set.seed(4)
sumstat <- groupwiseMedian(data = fold_changes.cells, var = "fold_change",
                           group = "tau", bca = TRUE, R = 10^5)
write_csv(sumstat, "fkbp_median_summary.csv")
...
ggsave("foldchanges.pdf", height = 3.5, width = 5)
