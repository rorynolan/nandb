...
1. In R, run
```{r Install nandb for Linux, eval=FALSE}
...
1. In R, run
```{r Install nandb for Windows, eval=FALSE}
install.packages("nandb")
```
...
In R, run
```{r Install nandb for Mac, eval=FALSE}
install.packages("nandb")
```
...
```{r Install nandb dev, eval=FALSE}
devtools::install_github("rorynolan/nandb")
```

## Use

For the lowdown on how to use this package, you should read the package [vignette](https://cran.r-project.org/package=nandb/vignettes/nandb.html). Once you've installed the package, you can browse them using `vignette(package = "nandb")`.
