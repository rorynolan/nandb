...
eg.img <- system.file("extdata", "50.tif", package = "nandb") %>% 
  ReadImageData
...
Finally, just in case you don't believe me that "the brightnesses without detrend and those with detrends of $\tau = `r round(tau)`$ and tau = `r round(tau.again)` roughly overlap, but the detrend of `tau = 10` has a significant shift," I'll \emph{prove} it to you by checking their values at 1.5. 
```{r PS}
nandb:::Closest(1.5, plot.df$brightness) %>% {filter(plot.df, brightness == .)}
```
