...
First, let's load the `nandb` and `EBImage` libraries and the `tidyverse` (we'll need them).
```{r load library, results='hide', message=FALSE}
required <- c("nandb", "EBImage", "tidyverse")
...
```
So we can see that `img` is a 3-dimensional array of 50 slices each of which is a $`r nrow(img)` \times `r ncol(img)`$ image. We can view the first slice:
```{r display image, fig.width=5, fig.height=5}
...
```{r Brightness calculation with bleaching correction, fig.width=5, fig.height=5}
...
```{r Brightness Plot, fig.width=5, fig.height=5}
...
```{r brightness density, message=FALSE, fig.width=5, fig.height=5}
...
```{r immobile plot, fig.width=5, fig.height=5}
...
```{r kmer plot, fig.width=5, fig.height=5}
